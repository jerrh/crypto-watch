  ...
  refreshCoins() {
    ...
    loader.present().then(() => {
      this.storage.get('likedCoins').then((val) => {
        // If storage value of likedCoins property not set when app first loads, push coins into likedCoins array
        if (!val) {
          this.likedCoins.push('BTC', 'ETH', 'XRP', 'EOS', 'BCH', 'LTC', 'TRX', 'ETC', 'ONT', 'IOT', 'NEO', 'ADA');
        ...
        // Otherwise, value of likedCoins property is set
        else {
          this.likedCoins = val;
